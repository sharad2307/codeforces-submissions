...
int dfs(ll node, ll p)
{
	...
	for(ll child:v[node])
	{
		if(child!=p)
		{
			...
			ll curr=1+dfs(child,node);
			diameter=max(diameter,curr+length);
...
{	
	...
	if(2*da>=min(diameter,db) || depth[b] <= da)cout<<"Alice"<<nl;
	else cout<<"Bob"<<nl;
}
