...
ll ansofdigit(long ll n){ll ans=0;while(n){ans=ans+n%10;n=n/10;}return ans;}
...
int main()
{
	...
	loop(i,0,7)
	{
		ll p=0,q=0,r=0,s=0,index=i;
		ll tt=1;
		while(tt)
		{
			if( index==0 || index==3 || index==6 ) p++;
			if( index==1 || index==5 ) q++;	
			if( index==2 || index==4 ) r++;
			if( r<=c && q<=b && p<=a) s++;
			else break;
			index=(index+1)%7;
		}
