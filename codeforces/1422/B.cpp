...
{	
	...
	loop(i,0,n)
	{
		loop(j,0,m)
		{
			ll x,y,z,p,fff=0,keep;
			x=a[i][j];
			y=a[i][m-1-j];
			z=a[n-i-1][j];
			p=a[n-i-1][m-j-1];

			fff=min(abs(x-y)+abs(x-z)+abs(x-p),abs(z-y)+abs(x-z)+abs(z-p));
			flag=1;
			fff=min(fff,abs(x-y)+abs(y-z)+abs(y-p));
			fff=min(fff,abs(x-p)+abs(y-p)+abs(z-p));
			flag=1;
			if(fff==abs(x-y)+abs(x-z)+abs(x-p) && flag) keep=x;
			else if(fff==abs(z-y)+abs(x-z)+abs(z-p)) keep=z;
			else if(abs(x-y)+abs(y-z)+abs(y-p)==fff) keep=y;
			else keep=p;
			if(i==n-i-1) fff/=2;
			if(j==m-j-1) fff/=2;
			if(a[i][j]!=keep && flag)
				a[i][j]=keep;
			if(a[i][m-1-j]!=keep && flag)
				a[i][m-1-j]=keep;
			if(a[n-i-1][j]!=keep && flag)
				a[n-i-1][j]=keep;
			if(a[n-i-1][m-j-1]!=keep && flag)
				a[n-i-1][m-j-1]=keep;
			ans+=fff;
		}
	}
