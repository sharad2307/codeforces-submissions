...
bool ctprime(ll num) { if(num < 2) return false; for(ll i = 2; i * i <= num; i++){ if(num % i == 0) return false; } return true; }
...
void solve()
{
	...
	ll flag=1;
		...
		ll ct = 0;
		ll s2 = 0;
		loop(i,0,n) s2 += abs(a[i] - b[i]);
		if (2 * s2 <= sum) ct = 1;
		if (ct == 1) loop(i,0,n) cout << b[i] << " ";
		else loop(i,0,n) cout << c[i] << " ";
		cout << nl;
}
